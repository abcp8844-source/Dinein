...
export default function OwnerWallet() {
  ...
  const handlePurchasePromotion = (cost) => {
    ...
    if (balance >= cost) {
      ...
      Alert.alert("Registry Sync Notice", `Insufficient ${currency} in your ${marketISO} account.`);
    }
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Animatable.View animation="fadeInDown" style={styles.header}>
          ...
          <View style={styles.syncStatus}>
             <Text style={styles.syncStatusText}>● SECURE DATA NODE: {userData?.idVerification || 'VERIFIED'}</Text>
          </View>
        </Animatable.View>
        ...
        <View style={styles.section}>
          ...
          >
            <View style={{flex: 1}}>
              ...
              <Text style={styles.promoDesc}>Priority placement in {country} store listings for 24h</Text>
            </View>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>RECENT SETTLEMENTS ({marketISO})</Text>
...
const styles = StyleSheet.create({
  ...
  badge: { backgroundColor: '#1A1A1A', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 4, marginBottom: 15, borderWidth: 1, borderColor: '#333' },
  badgeText: { color: '#D4AF37', fontSize: 7, fontWeight: '900', letterSpacing: 1 },
  ...
  syncStatus: { marginTop: 15, paddingHorizontal: 10, paddingVertical: 4, backgroundColor: '#0A0A0A', borderRadius: 4 },
  syncStatusText: { color: '#00FF00', fontSize: 7, fontWeight: 'bold', opacity: 0.8 },
  ...
  promoCard: { backgroundColor: '#0A0A0A', padding: 20, borderRadius: 15, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderWidth: 1, borderColor: '#111' },
  ...
  ledgerCard: { backgroundColor: '#050505', borderRadius: 15, padding: 20, borderWidth: 1, borderColor: '#111' },
