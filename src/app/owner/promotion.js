...
export default function OwnerPromotion() {
  ...
  const handleRequestPromotion = () => {
    if (!selectedItem) {
      Alert.alert("System Notification", "Please select a product to initialize the promotion protocol.");
      return;
    }
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: () => Alert.alert("Success", "Request transmitted to Admin for regional review.") 
        }
      ]
    );
  };
  ...
  const renderItem = ({ item }) => (
    ...
    >
      ...
      {selectedItem?.id === item.id && (
        <Text style={[styles.selectedLabel, { color: colors.primary }]}>SELECTED FOR BOOST</Text>
      )}
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: '#000' }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Escalate visibility in the {userData?.countryName} region.</Text>
      </View>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <Text style={styles.sectionLabel}>TARGET MARKET: {region} INVENTORY</Text>
        }
        ListEmptyComponent={
          <Text style={styles.emptyText}>No products available for promotion in this region.</Text>
        }
      />
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 10, fontWeight: 'bold', letterSpacing: 2, color: '#444', marginBottom: 20 },
  itemCard: { padding: 25, borderRadius: 20, marginBottom: 15, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  cardInfo: { flex: 1 },
  ...
  footer: { padding: 25, borderTopWidth: 0.5, borderTopColor: '#222', backgroundColor: '#000' },
  emptyText: { color: '#333', textAlign: 'center', marginTop: 50, letterSpacing: 1, fontSize: 12 }
});
