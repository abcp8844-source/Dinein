...
import { collection, query, where, onSnapshot, orderBy } from "firebase/firestore"; // Added for Real Sync
...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const total = ordersList.reduce((sum, order) => sum + (Number(order.price) || 0), 0);
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity
            ...
            style={[styles.statusToggle, { backgroundColor: isOnline ? "#06402B" : "#4D0F0F" }]}
          >
        ...
        <Animatable.View animation="fadeInUp" style={[styles.revenueCard, { backgroundColor: THEME.card }]}>
          ...
          <Text style={[styles.revenueAmount, { color: THEME.accent }]}>
            ...
            <Text style={styles.currency}>{userData?.currencyCode || "THB"}</Text>
          </Text>
        ...
        <View style={styles.actionGrid}>
          ...
          >
            ...
            <Text style={[styles.actionLabel, { color: THEME.textSecondary }]}>MY WALLET</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.orderSection}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={[styles.orderCount, { color: THEME.accent }]}>{orders.length} ACTIVE</Text>
          </View>
          ...
          {loading ? (
            ...
            orders.map((order) => (
              <Animatable.View key={order.id} animation="fadeInLeft" style={[styles.orderCard, { backgroundColor: THEME.card }]}>
                <View style={styles.orderMeta}>
                  <Text style={[styles.orderId, { color: THEME.textSecondary }]}>#{order.id.slice(-5)}</Text>
                  <Text style={[styles.orderTime, { color: THEME.accent }]}>ACTIVE</Text>
                </View>
                <Text style={[styles.orderItem, { color: THEME.textMain }]}>{order.itemName}</Text>
          ...
          {!loading && orders.length === 0 && (
            <Text style={{color: THEME.textSecondary, textAlign: 'center', marginTop: 20}}>No orders yet today.</Text>
          )}
        </View>
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  revenueCard: { marginHorizontal: 25, padding: 30, borderRadius: 25, borderWidth: 1, borderColor: "rgba(255,255,255,0.05)" },
  ...
  aiBriefing: { flexDirection: "row", alignItems: "center", marginTop: 15, borderTopWidth: 1, paddingTop: 15 },
  ...
  actionBox: { width: "48%", padding: 25, borderRadius: 20, alignItems: "center", borderWidth: 1, borderColor: "rgba(255,255,255,0.05)" },
  ...
  orderCard: { padding: 20, borderRadius: 20, marginBottom: 15, borderWidth: 1, borderColor: "rgba(255,255,255,0.05)" },
