  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} showsVerticalScrollIndicator={false}>
      ...
      <View style={[styles.box, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
        ...
        <Text style={[styles.text, { color: colors.textMain }]}>{response || "System Ready."}</Text>
        {escalate && (
      ...
      <View style={styles.inputRow}>
        ...
        <TouchableOpacity style={[styles.btnSend, { backgroundColor: colors.primary }]} onPress={handleSupportRequest} disabled={loading}>
          {loading ? <ActivityIndicator color="#000" /> : <Ionicons name="flash" size={18} color="#000" />}
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  input: { flex: 1, borderWidth: 0.5, borderRadius: 12, padding: 12, fontSize: 13 },
  btnSend: { width: 50, height: 50, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  btnAdmin: { marginTop: 15, padding: 10, borderRadius: 8, alignItems: 'center' },
