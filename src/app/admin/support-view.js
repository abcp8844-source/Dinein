  ...
  const renderTicket = ({ item }) => (
    <Animatable.View animation="fadeInLeft" style={styles.ticketCard}>
      <View style={styles.cardHeader}>
        ...
        <View style={[styles.roleBadge, { backgroundColor: item.role === 'owner' ? '#1B3921' : '#111' }]}>
          <Text style={[styles.roleText, { color: item.role === 'owner' ? '#4ADE80' : colors.primary }]}>
      ...
      <Text style={styles.issueText}>{item.issueType || "System Assistance"}</Text>
      
      <View style={styles.locationRow}>
        <MaterialCommunityIcons name="earth" size={12} color="#444" />
        <Text style={styles.locationText}>{item.region || 'Global Market'}</Text>
      </View>
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>No active support requests detected.</Text>
          }
        />
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  ticketCard: { backgroundColor: '#050505', padding: 20, borderRadius: 20, borderWidth: 1, borderColor: '#111', marginBottom: 15 },
  ...
  locationText: { color: '#444', fontSize: 10, marginLeft: 5, fontWeight: '900' },
  actionBtn: { marginTop: 20, height: 45, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  btnText: { color: '#000', fontSize: 10, fontWeight: '900', letterSpacing: 1 },
  emptyText: { color: '#111', textAlign: 'center', marginTop: 100, fontWeight: '900', fontSize: 12 }
});
