...
export default function ManageOwners() {
  ...
  const togglePartnerStatus = (id, currentStatus) => {
    ...
    Alert.alert(
      "Partner Control",
      `Are you sure you want to ${nextAction} this owner?`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Confirm",
          onPress: () =>
            console.log(`Partner ${id} status changed to ${nextAction}`),
        },
      ],
    );
  };
  ...
  const renderPartner = ({ item }) => (
    <View
      style={[
        styles.card,
        { borderColor: item.pendingHelp ? colors.primary : "#111" },
      ]}
    >
      <View style={styles.partnerDetails}>
        ...
        {item.pendingHelp && (
          <View
            style={[styles.alertBadge, { backgroundColor: colors.primary }]}
          >
      ...
      <View style={styles.actionRow}>
        <TouchableOpacity
          style={[
            styles.statusBtn,
            {
              backgroundColor: item.status === "active" ? "#1a1a1a" : "#dc3545",
            },
          ]}
        ...
        >
          <Text
            style={[
              styles.btnText,
              { color: item.status === "active" ? "#FFF" : "#FFF" },
            ]}
          >
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>
            No partners registered in this region.
          </Text>
        }
      />
    </SafeAreaView>
