  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={[styles.ticketCard, { borderColor: item.status === 'pending' ? '#D4AF37' : '#1B2631' }]}>
            <View style={styles.cardHeader}>
              <Text style={styles.userId}>USER ID: {item.userId?.substring(0, 8)}...</Text>
              <View style={[styles.badge, { backgroundColor: item.status === 'pending' ? '#FF3B30' : '#4CAF50' }]}>
                <Text style={styles.badgeText}>{item.status?.toUpperCase()}</Text>
              </View>
            </View>

            <Text style={styles.issueText}>{item.message || "Requesting human intervention."}</Text>
            
            <View style={styles.actionRow}>
              <TouchableOpacity 
                style={styles.contactBtn}
                onPress={() => Linking.openURL(`mailto:${item.userEmail || "support@example.com"}`)}
              >
                <MaterialCommunityIcons name="email-outline" size={16} color="#000" />
                <Text style={styles.btnText}>CONTACT USER</Text>
              </TouchableOpacity>

              {item.status === 'pending' && (
                <TouchableOpacity style={styles.resolveBtn} onPress={() => closeTicket(item.id)}>
...
const styles = StyleSheet.create({
  ...
  ticketCard: { backgroundColor: "#0A1A2F", padding: 15, borderRadius: 15, marginBottom: 15, borderWidth: 1 },
  ...
  contactBtn: { flex: 1, backgroundColor: "#D4AF37", padding: 12, borderRadius: 10, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8 },
  resolveBtn: { flex: 1, backgroundColor: "#1B2631", padding: 12, borderRadius: 10, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8, borderWidth: 1, borderColor: '#D4AF37' },
