...
export default function Register() {
  ...
  const handleRegistration = async () => {
    ...
    if (role === "owner" && !idNumber) {
      Alert.alert("COMPLIANCE ERROR", "Passport or Local ID is required for Merchant verification.");
      return;
    }
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: "#000" }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <Animatable.View animation="fadeInDown" style={styles.header}>
          <MaterialCommunityIcons name="shield-check-outline" size={50} color="#D4AF37" />
        ...
        <View style={styles.tabContainer}>
          <TouchableOpacity onPress={() => setRole("customer")} style={[styles.tab, role === "customer" && styles.activeTab]}>
            <Text style={[styles.tabText, { color: role === "customer" ? "#D4AF37" : "#444" }]}>CUSTOMER</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setRole("owner")} style={[styles.tab, role === "owner" && styles.activeTab]}>
            <Text style={[styles.tabText, { color: role === "owner" ? "#D4AF37" : "#444" }]}>MERCHANT</Text>
          </TouchableOpacity>
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.marketScroll}>
          {GLOBAL_MARKETS.map((m) => (
            <TouchableOpacity key={m.id} onPress={() => setSelectedCountry(m)} style={[styles.chip, selectedCountry.id === m.id && { borderColor: "#D4AF37", backgroundColor: "#111" }]}>
              <Text style={[styles.chipText, { color: selectedCountry.id === m.id ? "#D4AF37" : "#444" }]}>{m.name}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
        ...
        {role === "owner" && (
          <Animatable.View animation="fadeIn" style={styles.verificationBox}>
            ...
            <View style={styles.toggleRow}>
              <TouchableOpacity onPress={() => setVerificationType("local")} style={[styles.toggleBtn, verificationType === "local" && styles.toggleActive]}>
                <Text style={styles.toggleBtnText}>{selectedCountry.localIdName}</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => setVerificationType("passport")} style={[styles.toggleBtn, verificationType === "passport" && styles.toggleActive]}>
            ...
            <TextInput 
              style={styles.input} 
              placeholder={verificationType === "local" ? `${selectedCountry.localIdName} Number` : "Passport Identification Number"} 
              placeholderTextColor="#333" 
              value={idNumber} 
            ...
            <Text style={styles.legalNote}>* Documents must be valid in the {selectedCountry.name} operational node.</Text>
          </Animatable.View>
        )}

        <TouchableOpacity style={styles.registerBtn} onPress={handleRegistration}>
          <Text style={styles.registerBtnText}>INITIALIZE {role.toUpperCase()} PROFILE</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  title: { color: "#FFF", fontSize: 18, fontWeight: "900", letterSpacing: 4, marginTop: 15 },
  goldLine: { width: 40, height: 2, backgroundColor: "#D4AF37", marginTop: 10 },
  tabContainer: { flexDirection: "row", marginBottom: 30, backgroundColor: "#0A0A0A", borderRadius: 15, padding: 5 },
  tab: { flex: 1, paddingVertical: 15, alignItems: "center", borderRadius: 10 },
  activeTab: { backgroundColor: "#111", borderWidth: 0.5, borderColor: "#D4AF3733" },
  tabText: { fontSize: 10, fontWeight: "900", letterSpacing: 1.5 },
  label: { color: "#D4AF37", fontSize: 9, fontWeight: "900", letterSpacing: 2, marginBottom: 15 },
  marketScroll: { marginBottom: 30 },
  chip: { paddingHorizontal: 15, paddingVertical: 8, borderRadius: 8, borderWidth: 1, borderColor: "#111", marginRight: 10 },
  chipText: { fontSize: 10, fontWeight: "bold" },
  input: { height: 55, borderBottomWidth: 1, borderBottomColor: "#111", color: "#FFF", marginBottom: 20, fontSize: 14 },
  verificationBox: { backgroundColor: "#0A0A0A", padding: 20, borderRadius: 20, marginBottom: 25, borderWidth: 1, borderColor: "#111" },
  toggleRow: { flexDirection: "row", gap: 10, marginBottom: 20 },
  toggleBtn: { flex: 1, height: 40, justifyContent: "center", alignItems: "center", borderRadius: 10, borderWidth: 1, borderColor: "#222" },
  ...
  registerBtn: { backgroundColor: "#D4AF37", height: 60, borderRadius: 15, justifyContent: "center", alignItems: "center", marginTop: 10 },
  registerBtnText: { color: "#000", fontWeight: "900", letterSpacing: 2, fontSize: 12 },
});
