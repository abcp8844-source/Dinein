...
export default function Register() {
  ...
  const handleRegistrationInitiation = async () => {
    if (!form.email || !form.password || !form.idNumber || !form.phoneNumber) {
      Alert.alert("VALIDATION ERROR", "All identification fields must be completed.");
      return;
    }
  ...
  const finalizeRegistration = async () => {
    ...
    try {
      ...
      router.replace(form.role === "owner" ? "/(owner)/owner-wallet" : "/(customer)/home");
    } catch (error) {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      <Text style={[styles.title, { color: colors.primary }]}>ACCOUNT REGISTRATION</Text>

      {step === 1 ? (
        <View>
          <PremiumInput placeholder="Legal Name" value={form.fullName} onChangeText={(t) => setForm({ ...form, fullName: t })} />
          <PremiumInput placeholder="Email Address" value={form.email} onChangeText={(t) => setForm({ ...form, email: t })} />
          <PremiumInput placeholder="Secure Password" value={form.password} secureTextEntry onChangeText={(t) => setForm({ ...form, password: t })} />
          
          <View style={styles.marketSelection}>
            <Text style={{ color: colors.primary, fontSize: 12 }}>Market: {form.selectedCountry.name}</Text>
          </View>

          <PremiumInput placeholder="Phone (Intl Format)" value={form.phoneNumber} onChangeText={(t) => setForm({ ...form, phoneNumber: t })} keyboardType="phone-pad" />
          <PremiumInput placeholder={form.verificationType === "passport" ? "Passport No." : form.selectedCountry.localIdName} value={form.idNumber} onChangeText={(t) => setForm({ ...form, idNumber: t })} />
          
          <PremiumButton title={loading ? "Generating OTP..." : "Verify Identity"} onPress={handleRegistrationInitiation} />
        </View>
      ) : (
        <View>
          <PremiumInput placeholder="6-Digit Code" value={otpCode} onChangeText={setOtpCode} keyboardType="number-pad" maxLength={6} />
          <PremiumButton title={loading ? "Finalizing..." : "Complete Setup"} onPress={finalizeRegistration} />
        </View>
      )}
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 20, fontWeight: "800", textAlign: "center", marginBottom: 30, letterSpacing: 1.5 },
  marketSelection: { padding: 12, marginVertical: 10, borderLeftWidth: 4, borderLeftColor: "#D4AF37", backgroundColor: "rgba(212, 175, 55, 0.05)" },
});
