...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    if (!email || !password) {
      Alert.alert("Required", "Access credentials are missing.");
      return;
    }
    ...
    try {
      ...
      Alert.alert("Access Denied", "System could not verify your credentials.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.headerArea}>
        ...
        <Text style={[styles.tagline, { color: colors.textDim }]}>PREMIUM DELIVERY & DINE-IN</Text>
      </View>
      ...
      <View style={styles.footerLinks}>
        <TouchableOpacity onPress={() => router.push('/auth/register')}>
          <Text style={{ color: colors.textDim }}>NEW PARTNER? <Text style={{ color: colors.secondary }}>REGISTER</Text></Text>
        </TouchableOpacity>
        
        <TouchableOpacity 
          style={{ marginTop: 20 }}
          onPress={() => Alert.alert("Support Center", "For login issues, contact: abcp8844@gmail.com")}
        >
          <Text style={{ color: colors.primary, fontSize: 11, letterSpacing: 1 }}>SYSTEM HELP & SUPPORT</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  logoText: { fontSize: 36, fontWeight: '300', letterSpacing: 5 },
  underline: { height: 1, width: 30, marginVertical: 8 },
  tagline: { fontSize: 9, letterSpacing: 3, fontWeight: '500' },
  formArea: { marginBottom: 30 },
  inputWrapper: { borderBottomWidth: 1, marginBottom: 25, paddingBottom: 5 },
  label: { fontSize: 9, fontWeight: 'bold', letterSpacing: 2, marginBottom: 5 },
  input: { fontSize: 16, height: 40 },
  loginBtn: { height: 50, borderRadius: 2, justifyContent: 'center', alignItems: 'center' },
  btnText: { color: '#000', fontWeight: 'bold', fontSize: 12, letterSpacing: 2 },
