  ...
  useEffect(() => {
    const initializeChat = async () => {
      ...
      try {
        ...
        setMessages([{ id: "1", text: "PROTOCOL ACTIVE. HOW CAN I ASSIST YOU?", sender: "ai" }]);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: "#020B18" }]}>
      <View style={styles.header}>
        ...
        <MaterialCommunityIcons name="shield-key-outline" size={22} color="#FF3B30" />
      </View>
        ...
        <View style={styles.inputWrapper}>
          <View style={styles.inputContainer}>
            ...
            <TouchableOpacity onPress={handleSendMessage} disabled={loading} style={[styles.sendBtn, { backgroundColor: colors.primary }]}>
              {loading ? <ActivityIndicator size="small" color="#000" /> : <Ionicons name="send" size={18} color="#000" />}
            </TouchableOpacity>
          </View>
        </View>
      </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: 20, borderBottomWidth: 1, borderBottomColor: "#0A1A2F" },
  ...
  userBubble: { alignSelf: "flex-end", backgroundColor: "#D4AF37", borderBottomRightRadius: 2 },
  aiBubble: { alignSelf: "flex-start", backgroundColor: "#0A1A2F", borderBottomLeftRadius: 2, borderWidth: 1, borderColor: "#1B2631" },
  adminBubble: { alignSelf: "flex-start", borderColor: "#FF3B30", borderWidth: 1, backgroundColor: "#1A0505" },
  messageText: { fontSize: 13, lineHeight: 19, fontWeight: "500" },
  inputWrapper: { padding: 20, paddingBottom: Platform.OS === 'ios' ? 40 : 20, backgroundColor: "#020B18" },
  inputContainer: { flexDirection: "row", alignItems: "center", backgroundColor: "#051121", borderRadius: 25, paddingHorizontal: 15, paddingVertical: 8, borderWidth: 1, borderColor: "#0A1A2F" },
