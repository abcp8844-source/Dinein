...
export default function CustomerLogin() {
  ...
  const handleLogin = async () => {
    if (!email || !password) {
      Alert.alert(
        "Access Protocol",
        "Identification credentials are required for network entry.",
      );
      return;
    }
    try {
      ...
      Alert.alert(
        "Registry Error",
        "Invalid credentials. Please verify your global account data.",
      );
    }
  };
    ...
    <View style={[styles.container, { backgroundColor: "#000" }]}>
      ...
      >
        ...
        <Text style={styles.subTitle}>
          Access the 15-Market Premium Network
        </Text>
