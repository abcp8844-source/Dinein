...
export default function GlobalHeader() {
  ...
  const currentMarket = MARKET_REGISTRY.find(m => m.iso === marketISO) || MARKET_REGISTRY[0];
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {MARKET_REGISTRY.map((item) => (
            ...
            >
              ...
              <Text style={[styles.nodeIso, marketISO === item.iso && { color: "#D4AF37" }]}>
                {item.iso}
              </Text>
            </TouchableOpacity>
          ))}
...
const styles = StyleSheet.create({
  headerBody: { backgroundColor: "#020B18", paddingTop: 10, borderBottomWidth: 1, borderBottomColor: "#1A2A3A" },
  topSection: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 20, marginBottom: 15 },
  branding: { flexDirection: "row", alignItems: "center" },
  flagLarge: { fontSize: 24, marginRight: 10 },
  brandTitle: { color: "#FFF", fontSize: 14, fontWeight: "200", letterSpacing: 4 },
  langTrigger: { alignItems: "flex-end" },
  langLabel: { color: "#5D6D7E", fontSize: 7, fontWeight: "900", letterSpacing: 1 },
  langText: { color: "#D4AF37", fontSize: 10, fontWeight: "bold", marginTop: 2 },
  marketBar: { paddingLeft: 20, marginBottom: 10 },
  barLabel: { color: "#5D6D7E", fontSize: 7, fontWeight: "900", marginBottom: 10, letterSpacing: 1 },
  node: { alignItems: "center", marginRight: 22, paddingBottom: 8, borderBottomWidth: 2, borderBottomColor: "transparent" },
